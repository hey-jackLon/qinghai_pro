...
<script>
import { getTourismChannelList, getChannelTimeAnalysis } from '@/api/travelroadmanager/index'
  ...
  methods: {
    change(e) {
      ...
      for (let i = 0; i < this.events.length; i++) {
        if (this.events[i].channelId === e.odRegion) {
          data = this.events[i]
        }
      }
      ...
      getChannelTimeAnalysis(params, data).then(res => {
        ...
        this.lineoption.title.text = `${data.channelName}客车流量对比分析`
        this.lineoption.xAxis.data = []
        for (let i = 0; i < res.length; i++) {
          this.lineoption.xAxis.data.push(res[i].GCSJ)
        }
        ...
        this.pieoption.title.text = `${data.channelName}[${params.startDate}]客车车型结构`
    ...
    handlerRowClick(e) {
      this.pieoption.title.text = this.pieoption.title.text.replace(/\[.*?\]/g, `[${e.GCSJ}]`)
      this.pieoption.series[0].data = []
      for (let i = 0; i < this.pieoption.legend.data.length; i++) {
        if (this.pieoption.legend.data[i] === '大客车') {
          this.pieoption.series[0].data.push({
